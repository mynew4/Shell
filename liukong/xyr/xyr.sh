...
function authentication() {
    echo -n -e "请输入云免流控™官方网址 [\033[32m $key \033[0m] ："
    read PASSWD
    readkey=$PASSWD
    if [[ ${readkey%%\ *} == $key ]]
    then
...
echo
echo -n -e "请输入网站名称(默认名称云加速):"
read webname
if [[ -z $webname ]]
	then
		webname=云加速
		echo -e '[\033[32m  已设置网站名字为:云加速  \033[0m]';
	else
		echo -e "[\033[32m  已设置网站名字为:$webname  \033[0m]";
fi
echo
echo -e "是否生成流控APP？[y/n]（回车默认生成，集群负载中的副机无需生成）"
read llwsapp
if [[ $llwsapp == "y" ]] || [[ $llwsapp == "Y" ]] || [[ $llwsapp == "" ]];then
		echo
		echo -n -e "请输入APP名称(默认:云加速器):"
		read app_name
	if [[ -z $app_name ]]
		then
			echo -e '[\033[32m  已设置App名称:云加速器  \033[0m]';
			app_name=云加速器
		else
...
------------------------------------------------------------
前台/用户中心，用户查流量的网址：http://${IP}:${port}/user
------------------------------------------------------------
