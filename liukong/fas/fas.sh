...
rpm -Uvh ${file_server}webtatic-release.rpm
...
#安装openvpn.rpm所需环境！
rpm -Uvh ${file_server}liblz4-1.8.1.2-alt1.x86_64.rpm
#这里使用的是非筑梦官方的openvpn源，由于筑梦官方的openvpn2.4.3的rpm包找不到，只能采用最新版的2.4.6的rpm包
rpm -Uvh ${file_server}openvpn-2.4.6-1.el7.x86_64.rpm
}
function ly03() {
...
wget -q ${file_server}dnsmasq.conf -P /etc && chmod 0777 /etc/dnsmasq.conf
if [ ! -f /etc/dnsmasq.conf ]; then
...
function web() {
#安装web云端
rm -rf /var/www/* && cd /var/www && wget -q ${file_server}fas_web.zip
...
#更新命令指示符
cd /root&&wget -q ${file_server}res.zip
...
cd /APP
wget -q ${file_server}fas.apk&&wget -q ${file_server}apktool.jar&&java -jar apktool.jar d fas.apk >/dev/null 2>&1&&rm -rf fas.apk
...
echo "---------------------------------------------"
echo "APP下载地址: http://"$IP":"$lyApacheport"/fasapp.apk"
echo "---------------------------------------------"
echo "---------------------------------------------"
exit;0
}
...
function logo() {
echo
echo -e "欢迎使用FAS网络用户管理系统"
echo
echo -e "本脚本为破解脚本，使用问题请自行解决"
echo
echo -e "Author: 十一 & 凌一"
pass="blog.67cc.cn";
echo -e "请输入十一博客地址:[\033[32m $pass \033[0m] "
read inputPass
...
version="006";
Crackauthor="十一 & 凌一";
file_server="https://coding.net/u/marisn/p/fas/git/raw/master/"
logo
}
main
echo '#盗版屏蔽dingd.cn
